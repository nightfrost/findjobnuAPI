...
public class CvReadabilityService : ICvReadabilityService
{
    ...
    public async Task<CvReadabilityResult> AnalyzeAsync(IFormFile pdfFile, CancellationToken cancellationToken = default)
    {
        ...
        string text;
        await using (var ms = new MemoryStream((int)pdfFile.Length))
        {
    ...
    private static string BuildSummary(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            return "Ingen tekst kunne udtrækkes. PDF'en kan være billedbaseret eller beskyttet.";
        }
        ...
        // Heuristikker: sektioner, punktopstillinger, kontaktinfo
        ...
        var sectionKeywords = new[] { "experience", "education", "skills", "projects", "summary", "profile", "erfaring", "uddannelse", "færdigheder", "projekter", "om", "om mig", "bio", "profil", "kontakt" };
        var matchedSections = sectionKeywords.Count(k => Regex.IsMatch(text, $@"(^|\n)\s*{Regex.Escape(k)}\b", RegexOptions.IgnoreCase));

        var sb = new StringBuilder();
        sb.AppendLine($"Tegn: {totalChars}");
        sb.AppendLine($"Ord: {totalWords}");
        sb.AppendLine($"Linjer: {totalLines}");
        sb.AppendLine($"E-mail fundet: {(hasEmail ? "ja" : "nej")}");
        sb.AppendLine($"Telefonnummer fundet: {(hasPhone ? "ja" : "nej")}");
        sb.AppendLine($"Punktopstillinger: {bulletCount}");
        sb.AppendLine($"Genkendte sektioner: {matchedSections} / {sectionKeywords.Length}");
